...
table = dynamodb.Table('ALB_NODE_IP_ADDRESS_INFO')
...
def describe_target_health(tg_arn):
    ...
    try:
        ...
        print("method called <describe_target_health> with NLB Target Group " + tg_arn)
        ...
        print(e.response['Error']['Message'])
    print("Returning Registered IP list {0}".format(registered_ip_list))
...
def dns_lookup(domainname, record_type, *dnsserver):
    ...
    """
    print("dns_lookup called with domainname " + domainname)
    print("dns_lookup called with record_type " + record_type)
    lookup_result_list = []
    myResolver = dns.resolver.Resolver()
    if not dnsserver:
        lookupAnswer = myResolver.query(domainname, record_type)
        print("Lookup Answer --- > {}".format(lookupAnswer))
    else:
        myResolver.nameservers = random.choice(dnsserver)
        lookupAnswer = myResolver.query(domainname, record_type)
    for answer in lookupAnswer:
        print("appending answer --- > {}".format(answer))    
        lookup_result_list.append(str(answer))
    print("lookup result list ----> {}".format(lookup_result_list))
...
def lambda_handler(event, context):
    ...
    print("regional name of ALB DNS is " + regional_name)

    authoritative_server_ip_list = []
    regular_record_set = []
    print("Calling method <describe_target_health>******")
    ...
    for nameserver_domain in authoritative_server_domain_list:
        print("Nameserver Domain " + nameserver_domain)
