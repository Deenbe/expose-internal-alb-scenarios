...
delete_infra_stack() {
    stack_name=$1
    echo "Deleting Cloud Formation stack: \"${PROJECT_NAME}-cons-infra\"..."
    aws cloudformation delete-stack --stack-name ${PROJECT_NAME}-cons-infra
    echo 'Waiting for the stack to be deleted, this may take a few minutes...'
    aws cloudformation wait stack-delete-complete --stack-name ${PROJECT_NAME}-cons-infra
    echo 'Done'
}
