...
deploy_httpapi(){
   echo "Deploying Cloud Formation stack: \"${PROJECT_NAME}-httpapi\" containing HTTP API..."
    aws cloudformation deploy \
        --no-fail-on-empty-changeset \
        --stack-name "${PROJECT_NAME}-httpapi" \
        --template-file "${DIR}/http-api-stack/cf-private-api-stack.yaml" \
        --capabilities CAPABILITY_IAM \
        --parameter-overrides "ProjectName=${PROJECT_NAME}"
}
deploy_stacks() {
...
delete_stacks() {
    delete_cfn_stack "${PROJECT_NAME}-httpapi"
